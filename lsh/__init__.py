...
class LSHCache:
    ...
    def __init__(self, n=512, b=32, r=16, max_shingle=3):
        # assign it
        self._n = n
        self._b = b
        self._r = r
    ...
    def _insert_lsh(self,lsh,doc_id,date_added):
        ...
        if (doc_id in self._seen):
            ...
            if(doc_id < 0):
                ...
                for i,band_bucket in enumerate(lsh):
                    #print('inside insert',i,band_bucket)

                    if doc_id not in self._cache[i][band_bucket]:
                        dup_buckets.append(self._cache[i][band_bucket])
                        #把每个band哈希到一些bucket中，注意bucket的数量要足够多，i：bands数量索引  每个bands包含足够数量多的buckets
                        #hashtable里只存放数据id
    ...
    def prepare_dup_buckets(cls, buckets, id=None):
        ...
        '''
        dengxi = [[1,2,3], [4,5,6], [7,8,9]]
        dengxi_new = reduce(list.__add__, dengxi, [])
        print('**** ', dengxi_new)
        '''
        #print('all:  ',all)
        #for search:
    ...
    def insert_sig(self, sig, id, date_added=int(time.time()), passive=True):
        lsh = self._get_lsh(sig, self._b,self._r)
        logging.debug('id: %d lsh: %s', id, lsh)
        dup_buckets = self._insert_lsh(lsh, id, date_added)
        return self.prepare_dup_buckets(dup_buckets, id=id)
